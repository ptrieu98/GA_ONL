...
public class PlayerMovement : MonoBehaviour
{
    ...
    [Header("Photon Setup")]
    // Kéo Camera con của Player vào đây
    public Camera playerCamera; 
    // Kéo AudioListener con của Player vào đây
    ...
    void Start()
    {
        ...
        // === QUAN TRỌNG: TẮT CAMERA CỦA NGƯỜI KHÁC ===
        // Nếu nhân vật này KHÔNG phải của tôi
        if (!photonView.IsMine)
        {
            // Tắt Camera để không bị chiếm màn hình
            if (playerCamera != null) 
            {
                playerCamera.enabled = false;
            }
            
            // Tắt AudioListener để không nghe âm thanh từ vị trí họ
    ...
    void Update()
    {
        // === QUAN TRỌNG: CHẶN ĐIỀU KHIỂN ===
        // Nếu nhân vật này không phải của tôi -> DỪNG LẠI NGAY
        if (photonView != null && !photonView.IsMine)
        {
            return; 
        }

        // --- Code bên dưới chỉ chạy cho Player của chính mình ---
    ...
    {
        // Chỉ phát âm thanh bước chân nếu là nhân vật của tôi
